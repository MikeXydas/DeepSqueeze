"""Code taken and slightly modified from https://github.com/lucidrains/mixture-of-experts"""
import torch
...
class MoE(nn.Module):
    ...
    def get_codes(self, inputs):
        ...
        orig_shape = expert_inputs.shape
        expert_inputs = expert_inputs.reshape(e, -1, d)
        expert_codes = self.experts.expert_encoders(expert_inputs)
        # expert_codes_r = expert_codes.reshape(*orig_shape[:-1], 1)
        # out_codes = torch.einsum('ebcd,bnec->bnd', expert_codes_r, combine_tensor)

        return expert_codes, orig_shape, combine_tensor

    def get_recons_from_codes(self, codes, orig_shape, combine_tensor):
        expert_recons = self.experts.expert_decoders(codes)
        expert_recons = expert_recons.reshape(*orig_shape)
        expert_recons = torch.einsum('ebcd,bnec->bnd', expert_recons, combine_tensor)

        return torch.squeeze(expert_recons)